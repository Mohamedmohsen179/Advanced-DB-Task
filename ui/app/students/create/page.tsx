...
export default function CreateStudentPage() {
  ...
  const validate = (): boolean => {
    ...
    const ssnError = validators.required(formData.stuSsn);
    if (ssnError) newErrors.stuSsn = ssnError;
    ...
    const dobError = validators.required(formData.dob);
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setFormData((prev) => ({
      ...prev,
      [name]: name === 'level' || name === 'deptId' ? parseInt(value) || 0 : value,
    }));
    if (errors[name]) {
      setErrors((prev) => {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="mb-6">
        <Link href="/students" className="text-blue-600 hover:text-blue-800 dark:text-blue-400">
          ← Back to Students
        </Link>
      </div>

      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
        <h1 className="text-3xl font-bold mb-6 text-gray-900 dark:text-white">Create New Student</h1>

        <form onSubmit={handleSubmit} className="space-y-4">
          <FormField
            label="SSN"
            name="stuSsn"
            value={formData.stuSsn}
